...
def transformation(
    ...
    make_fixed_points_file(coords[["x", "y", "z"]].values, "temp.dat")
    transformix_img_filt.SetFixedPointSetFileName("temp.dat")
...
def transformix_file_to_coords(output_points_fp):
    ...
    df.columns = df.loc[0].str.strip().str.split("\s").str[0]
