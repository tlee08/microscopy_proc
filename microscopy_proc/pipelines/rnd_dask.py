...
if __name__ == "__main__":
    ...
    # )
    # arr_dog = disk_cache(arr_dog, os.path.join(out_dir, "3_adaptive_filt.zarr"))
    ...
    arr_watershed = da.map_overlap(
        watershed_segm, arr_raw, arr_maxima, arr_filt, depth=depth
    )
    arr_watershed = disk_cache(arr_watershed, os.path.join(out_dir, "8_watershed.zarr"))

    # Step 9: Get coords of maxima and get corresponding sizes from watershed
    cell_coords = block_to_coords(region_to_coords, arr_maxima)
    cell_coords.to_parquet(os.path.join(out_dir, "9_cells.parquet"))
